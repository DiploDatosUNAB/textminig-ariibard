...
library(tidyverse) #Para usar dplyr
library(tidytext) #Para usar tidytext
library(wordcloud2)
library(tm)
library(readr)
library(purrr)
...
```{r}
titulos <- read_delim("rladies_meetups.csv", 
...
```{r}
idiomas <- list('spanish', 'portuguese')

# list('spanish', 'portuguese', 'french', 'danish', 'dutch', 'finnish', 'german', 'hungarian', 'italian', 'norwegian', 'russian', 'swedish')
...
```{r}
wordcloud2::wordcloud2(palabras , size = 1, minRotation = -pi/6, maxRotation = -pi/6,
